      ...
      <div className="flex flex-col gap-4">
        <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
          <nav aria-label="Footer Navigation" className="flex flex-wrap justify-center sm:justify-start gap-3 sm:gap-6 uppercase tracking-wider">
            ...
            <button onClick={() => setView('disclaimer')} className="hover:text-blue-900 transition-colors">Disclaimer</button>
        ...
        <div className="pt-3 border-t border-gray-200 text-center sm:text-left text-gray-400 max-w-4xl">
          <strong>Disclaimer:</strong> Tool results are estimates for informational purposes only. Do not rely on them as professional advice. <button onClick={() => setView('disclaimer')} className="underline hover:text-blue-900">Read our full disclaimer.</button>
        </div>
      </div>
    </footer>
  );
